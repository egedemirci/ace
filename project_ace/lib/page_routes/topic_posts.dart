...
class TopicPostsView extends StatefulWidget {
  const TopicPostsView({Key? key, required this.topic, required this.analytics}) : super(key: key);
  final Topic topic;
                ...
                if (!querySnapshot.hasData) {
                  ...
                  for (int j = 0; j < postsList.length; j++) {
                    for (int k = 0; k < postsList[j].length; k++) {
                      if(widget.topic.postIdList.contains(postsList[j][k]['postId'])){
