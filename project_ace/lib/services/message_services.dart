...
class MessageService{
  ...
  final CollectionReference chatRoomReference = FirebaseFirestore.instance.collection('ChatRooms');
  ...
  createMessage(String currentId, String otherId) async{
    ...
    if(!ds.exists) {
      await chatRoomReference.doc(bigger
      ).set(ChatRoom(
          chatRoomId: bigger,
          lastMessageCreatedAt: DateTime.now(),
          usersChatting: [currentId, otherId])
          .toJson()
      );
    }
  }
  ...
  // Send Message
  Future<void> sendMessage(String chatId, String message, String senderUsername, String senderAvatar) async
  {
    await chatRoomReference.doc(chatId).update({
      'texts': FieldValue.arrayUnion([Message(
          senderUsername: senderUsername,
          message: message,
          createdAt: DateTime.now(),
          urlAvatar: senderAvatar).toJson()]),
      'lastMessage': message,
